...
public class PlayerLetterHints
{
    ...
    public void CalculatePlayerLetterHints()
    {
        ...
        var solutionCopy = new List<char>(_game.Solution).Select((letter,idx) => new {letter,idx}).ToList();
        var test = _game.Solution.Select((letter, idx) => new SolutionLetterRecord(letter, idx)).ToList();
        ...
        {
            var letterIsCorrect = test.FirstOrDefault((letter) => letter.Letter.ToString().Equals(evaluation.Letter) && letter.Index == evaluation.WordIndex);
            if (letterIsCorrect is null) return;
            
            Correct.Add(evaluation);
            test.Remove(letterIsCorrect);
        });
        ...
        allLetterEvaluations.ForEach(evaluation =>
        {
            var letterIsWronglyPositioned =
                test.FirstOrDefault(letter => letter.Letter.ToString().Equals(evaluation.Letter));

            if (letterIsWronglyPositioned is null) return;
            WrongPosition.Add(evaluation);
            test.Remove(letterIsWronglyPositioned);
        });
